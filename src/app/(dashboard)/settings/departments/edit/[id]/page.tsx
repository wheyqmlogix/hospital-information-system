...
import { DepartmentForm } from "@/components/staff/department-form";
...
export default function EditDepartmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="p-20 text-center animate-pulse text-[10px] font-black uppercase tracking-widest text-slate-400">Loading Unit Specification...</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="space-y-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-[#0f172a] uppercase tracking-tight leading-none mb-3">Modify Department</h1>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Institutional Unit Hierarchy Update</p>
        </div>
      </div>
      ...
      <Card className="border-slate-200 shadow-sm rounded-sm overflow-hidden bg-white">
        <div className="px-8 py-5 bg-slate-50 border-b border-slate-200 flex items-center gap-3">
          <div className="h-6 w-6 bg-[#0f172a]/10 rounded-sm flex items-center justify-center">
            <Building2 className="h-3.5 w-3.5 text-[#0f172a]" />
          </div>
          <h2 className="text-[11px] font-black text-[#0f172a] uppercase tracking-widest">Update Specification</h2>
        </div>

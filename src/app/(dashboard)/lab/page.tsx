    ...
    <div className="max-w-[1400px] mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black text-[#0f172a] tracking-tighter flex items-center gap-3 uppercase">
             <FlaskConical className="h-8 w-8 text-[#0f172a]" />
             Laboratory Command Center
          </h1>
          <p className="text-slate-500 font-bold text-[11px] uppercase tracking-widest">Monitor diagnostic requests, sample collection, and result releasing.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "Total Load Today", value: stats.totalToday, icon: TrendingUp, color: "text-[#0f172a]", bg: "bg-slate-50" },
        ].map((stat, i) => (
      ...
      <Card className="border border-slate-200 rounded-sm overflow-hidden">
        <CardHeader className="bg-slate-50 border-b border-slate-100 px-8 py-6">
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <CardTitle className="text-sm font-black uppercase tracking-widest text-[#0f172a]">Diagnostic Ledger</CardTitle>
              ...
              <table className="w-full text-left">
                 <thead>
                    <tr className="border-b border-slate-100">
                       ...
                       <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Test Name</th>
                      ...
                      <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-8 py-5">
                           <div className="flex flex-col">
                              <span className="font-bold text-[#0f172a] font-mono text-xs uppercase">{order.orderNumber}</span>
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{new Date(order.createdAt).toLocaleTimeString()}</span>
                           </div>
                        </td>
                        <td className="px-8 py-5">
                           <div className="flex items-center gap-3">
                              <div className="h-7 w-7 rounded-sm bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400">
                                 <User className="h-3.5 w-3.5" />
                              </div>
                              <div className="flex flex-col">
                                 <span className="font-black text-[#0f172a] uppercase tracking-tight text-xs">{order.admission.patient.lastName}, {order.admission.patient.firstName}</span>
                                 <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{order.admission.patient.patientId}</span>
                              </div>
                           </div>
                        </td>
                        <td className="px-8 py-5">
                           <div className="flex flex-col">
                              <span className="font-bold text-[#0f172a] uppercase tracking-tight text-xs">{order.test.name}</span>
                              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{order.test.category}</span>
                           </div>
                        </td>
                        <td className="px-8 py-5 text-center">
                           <span className={cn(
                             ...
                             order.status === 'COLLECTED' ? "bg-slate-50 text-[#0f172a] border-slate-200" : 
                             order.status === 'PROCESSING' ? "bg-purple-50 text-purple-600 border-purple-100" : 
                             order.status === 'COMPLETED' ? "bg-green-50 text-green-600 border-green-100" : "bg-slate-50 text-slate-600 border-slate-200"
                           )}>
                              {order.status}
                           </span>
                        </td>
                        <td className="px-8 py-5 text-right">
                           <div className="flex items-center justify-end gap-2">
                              <Protected permission="manage_lab_results">
                                 {order.status === 'PENDING' && (
                                   <Button 
                                     ...
                                     className="bg-slate-50 hover:bg-slate-100 text-[#0f172a] border border-slate-200 rounded-sm h-7 px-4 text-[9px] font-black uppercase tracking-widest"
                                   >
                                 ...
                                 {order.status === 'COLLECTED' && (
                                   <Button 
                                     ...
                                     className="bg-slate-50 hover:bg-slate-100 text-purple-600 border border-purple-100 rounded-sm h-7 px-4 text-[9px] font-black uppercase tracking-widest"
                                   >
                                 ...
                                 {order.status === 'PROCESSING' && !editingResult && (
                                   <Button 
                                     ...
                                     className="bg-slate-50 hover:bg-slate-100 text-green-600 border border-green-100 rounded-sm h-7 px-4 text-[9px] font-black uppercase tracking-widest"
                                   >
                              ...
                              <Link href={`/admissions/${order.admission.id}`}>
                                 <Button size="sm" variant="outline" className="rounded-sm border-slate-200 text-[#0f172a] font-black uppercase tracking-widest text-[9px] h-7">
      ...
      {editingResult && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <Card className="w-full max-w-xl border border-slate-200 rounded-sm overflow-hidden animate-in zoom-in-95 duration-200">
              <CardHeader className="bg-slate-50 border-b border-slate-100 px-8 py-6">
              ...
              <CardContent className="p-8 space-y-6">
                 <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Clinical Observations & Findings</label>
                    <textarea
                      className="w-full h-48 rounded-sm border border-slate-200 p-4 focus:border-[#0f172a] focus:ring-0 font-bold text-xs transition-all placeholder:text-slate-300 placeholder:uppercase"
                 ...
                 <div className="flex gap-4">
                    <Button 
                      ...
                      className="flex-1 h-10 rounded-sm font-black uppercase tracking-widest text-xs"
                    >
                       Cancel
                    </Button>
                    <Button 
                      ...
                      className="flex-1 h-10 rounded-sm bg-[#0f172a] hover:bg-black text-white font-black uppercase tracking-widest text-xs"
                    >
                       Release Result
                    </Button>
                 </div>

...
import { X, Bed as BedIcon } from "lucide-react";
...
export function BedModal({ isOpen, onClose, onSuccess, roomId, initialData }: BedModalProps) {
  ...
  if (!isOpen || (!roomId && !initialData)) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = initialData ? `/api/admin/beds/${initialData.id}` : "/api/admin/beds";
      const method = initialData ? "PATCH" : "POST";
      const body = initialData ? { name } : { name, roomId };
      ...
      toast.success(`Bed ${initialData ? "updated" : "added"} successfully`);
      onSuccess();
      onClose();
    } catch (error: any) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[11px] font-black text-[#0f172a] uppercase tracking-widest">
                {initialData ? "Modify Bed Specification" : "Add Clinical Bed"}
              </h2>
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-tight">Facility Unit Configuration</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-1.5">
            <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center">
              <BedIcon className="h-3 w-3 mr-2" />
              Bed Name / Number
            </Label>
            <Input 
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Bed 01, NICU-05" 
          ...
          <div className="flex items-center justify-end space-x-3 pt-6 border-t border-slate-100">
            ...
            >
              {isSubmitting ? "Processing..." : initialData ? "Update Bed" : "Add Bed"}
            </Button>
          </div>
        </form>
      </div>
